...
func instantiateNode(id int) *Node {
	...
	node := Node{id, 0, nodeChannel, nodePQ, nil,
		replyHistory, &sync.WaitGroup{}, make(chan int)}
...
func (node *Node) enqueue(message Message) {
	...
	sort.SliceStable(node.nodePQ, func(i, j int) bool {
		...
		if node.nodePQ[i].timestamp < node.nodePQ[j].timestamp {
			return true
		} else if node.nodePQ[i].timestamp == node.nodePQ[j].timestamp && node.nodePQ[i].senderID < node.nodePQ[i].senderID {
			return true
		} else {
			return false
		}
...
func (node *Node) enterCS(msg Message) {
	...
	for _, req := range node.nodePQ {
		...
		release := Message{
			...
			senderID:     node.id,
			timestamp:    node.logicalClock,
			receiver:     Receiver{req.senderID, req.timestamp},
		}

		go node.sendmessage(release, req.senderID)
	}
...
func (node *Node) sendReply(msg Message) {
	...
	node.sendmessage(sendReply, msg.senderID)
}
...
func (node *Node) handleRequest(msg Message) {
	...
	for _, req := range node.nodePQ {
		...
		if req.senderID == node.id {
			if req.timestamp < msg.timestamp {
				earlierReq = true
				break
			} else if req.timestamp == msg.timestamp && req.senderID < msg.senderID {
				earlierReq = true
				break
			}
		}
